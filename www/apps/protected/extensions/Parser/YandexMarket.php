...
class Parser_YandexMarket {
    ...
    public static function parse($count, $proxyList, $pid, $iCount) {
//        var_dump($proxyList);exit;
        ...
        $successLoad  =false;
        while (!$successLoad) {
            $successLoad = $dom->load(Yii::app()->getBasePath().'/categories.xml');
        }
        $items = $dom->getElementsByTagName("category");
        
        $browsers = include Yii::app()->getBasePath().'/browsers.php';
...
//        echo $start." ".$end.PHP_EOL;exit;
        $db = Yii::app()->db;
        $c = $db->createCommand("SET NAMES 'utf8'; SET CHARACTER SET 'utf8';");
        $c->execute();
        for ($i = $start; $i < $end; $i++) {
...
//            $propValues[1];
    ...
    public static function compareGoods($ids, $cid, $hid, $db, &$ch, &$proxyList) {
        ...
        $goodInsertQueryStatement = $db->createCommand("INSERT into goods (`name`, `articul`, `source`) VALUES(:name, :articul, :source)");
        $propertyInsertQueryStatement = $db->createCommand("INSERT IGNORE INTO properties (`group_id`, `name`, `strid`) VALUES(:gid, :name, :strid)");
        $propValueInsertQueryStatement = $db->createCommand("INSERT INTO property_values (`prop_id`, `value`, `product_id`) VALUES(:propid, :val, :pid)");
        $propIdQueryStatement =  $db->createCommand("SELECT id FROM properties WHERE group_id=:gid AND strid=:stid");
            ...
            if ($info['http_code'] != 200) {
                ...
                foreach ($goodsWithDescriptions as $prodName => $properties) {
                    ...
                    foreach ($properties as $property) {
                        $propertyInsertQueryStatement->bindParam(":gid", $cid, PDO::PARAM_INT);
                        $propertyInsertQueryStatement->bindParam(":name", $property["pname"], PDO::PARAM_STR);
                        $propertyInsertQueryStatement->bindParam(":strid", $property["pid"], PDO::PARAM_STR);
                        $propertyInsertQueryStatement->execute();

                        $propId = intval($db->lastInsertId());
                        if ($prodId == 0) {
                            $propIdQueryStatement->bindParam(":gid", $cid, PDO::PARAM_INT);
                            $propIdQueryStatement->bindParam(":stid", $property['pid'], PDO::PARAM_STR);
                            $r = $propIdQueryStatement->queryAll();
                            var_dump($r);exit;
                            $propId = intval($r['id']);
                        }
                        $propValueInsertQueryStatement->bindParam(":propid", $propId, PDO::PARAM_INT);
                        $propValueInsertQueryStatement->bindParam(":val", $property['pvalue'], PDO::PARAM_STR);
                        $propValueInsertQueryStatement->bindParam(":pid", $gid, PDO::PARAM_INT);
                        $propValueInsertQueryStatement->bindParam();
...
//        echo getmypid()." - end category ".$cid.PHP_EOL;
    ...
    public static function packFeatures($totalCount, $threadsCount, $index) {
        ...
        $propertiesQueryStatement = $db->createCommand("SELECT p.name n, pv.value v FROM properties AS p JOIN property_values AS pv ON (pv.prop_id=p.id) WHERE pv.product_id=:pid");
        $goodUpdateQueryStatement = $db->createCommand("UPDATE goods SET features=:fe WHERE id=:pid");
        
        $sql = "SELECT id FROM goods WHERE source='market' LIMIT :limit OFFSET :offset";
//        echo $sql.PHP_EOL;exit;
        $stm = $db->createCommand($sql);
        $stm->bindParam(":limit", $limit, PDO::PARAM_INT);
        $stm->bindParam(":offset", $start, PDO::PARAM_INT);
        $rows = $stm->queryAll();
        foreach ($rows as $row) {
            $propertiesQueryStatement->bindParam(":pid", $row['id'], PDO::PARAM_INT);
            $properties = $propertiesQueryStatement->queryAll();
            $dom = new DOMDocument('1.0', 'utf-8');
